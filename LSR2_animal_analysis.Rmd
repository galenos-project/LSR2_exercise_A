...
For behavioural outcomes, an NMD was calculable for `r df_S_B %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_B)` comparisons, i.e. `r ((df_S_B %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_B)))*100` % of comparisons.

The effect of exercise on BDNF in animals using NMD as the effect size is shown in Figure 2.5.5. The pooled estimate for NMD across all individual comparisons is displayed as a diamond shape at the bottom of the plot. Dotted lines indicate the prediction interval of the pooled estimate.

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
NMD_S_B <- run_ML_NMD(df, "TvC", "BDNF", 0.5)
```

**Figure 2.5.5**

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height = 10}
forest_metafor_NMD(NMD_S_B, "BDNF")
```

Exercise had a pooled effect on BDNF of NMD = `r NMD_S_B[["beta"]][1]` (95% CI: `r NMD_S_B[["ci.lb"]]` to `r NMD_S_B[["ci.ub"]]`) with a prediction interval of `r predict(NMD_S_B)$pi.lb` to `r predict(NMD_S_B)$pi.ub`). For reference the pooled effect size for SMD was `r SMD_S_B[["SMD_ML"]][["beta"]][1]` (95% CI: `r SMD_S_B[["SMD_ML"]][["ci.lb"]]` to `r SMD_S_B[["SMD_ML"]][["ci.ub"]]`).

`r NMD_S_B[["k"]]` experimental comparisons were reported in `r NMD_S_B[["s.nlevels"]][3]` experiments reported from `r length(unique(NMD_S_B$data$StudyId))` publications and involving `r NMD_S_B[["s.nlevels"]][1]` different animal strains.

|            Level            |                                            Number of categories for that level included in this analysis                                            |                                                                Attributable variance                                                                |
|:-------------:|:---------------------------:|:---------------------------:|
|           Strain            |                        `r ifelse('Strain' %in% NMD_S_B$s.names, NMD_S_B$s.nlevels[[which(NMD_S_B$s.names == 'Strain')]], 0)`                        |                         `r ifelse('Strain' %in% NMD_S_B$s.names, NMD_S_B$sigma2[[which(NMD_S_B$s.names == 'Strain')]], NA)`                         |
|       Study x Strain        |                `r ifelse('Strain/StudyId' %in% NMD_S_B$s.names, NMD_S_B$s.nlevels[[which(NMD_S_B$s.names == 'Strain/StudyId')]], 0)`                |                 `r ifelse('Strain/StudyId' %in% NMD_S_B$s.names, NMD_S_B$sigma2[[which(NMD_S_B$s.names == 'Strain/StudyId')]], NA)`                 |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_B$s.names, NMD_S_B$s.nlevels[[which(NMD_S_B$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_B$s.names, NMD_S_B$sigma2[[which(NMD_S_B$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
For stress response outcomes, an NMD was calculable for `r df_S_StR %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_StR)` comparisons, i.e. `r ((df_S_StR %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_StR)))*100` % of comparisons.

The effect of exercise on stress response in animals using NMD as the effect size is shown in Figure 2.6.5. The pooled estimate for NMD across all individual comparisons is displayed as a diamond shape at the bottom of the plot. Dotted lines indicate the prediction interval of the pooled estimate.

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
NMD_S_StR <- run_ML_NMD(df, "TvC", "Stress response", 0.5)
```

**Figure 2.6.5**

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height = 6}
forest_metafor_NMD(NMD_S_StR, "Stress response")
```

Exercise had a pooled effect on stress response of NMD = `r NMD_S_StR[["beta"]][1]` (95% CI: `r NMD_S_StR[["ci.lb"]]` to `r NMD_S_StR[["ci.ub"]]`) with a prediction interval of `r predict(NMD_S_StR)$pi.lb` to `r predict(NMD_S_StR)$pi.ub`). For reference the pooled effect size for SMD was `r SMD_S_StR[["SMD_ML"]][["beta"]][1]` (95% CI: `r SMD_S_StR[["SMD_ML"]][["ci.lb"]]` to `r SMD_S_StR[["SMD_ML"]][["ci.ub"]]`).

`r NMD_S_StR[["k"]]` experimental comparisons were reported in `r NMD_S_StR[["s.nlevels"]][3]` experiments reported from `r length(unique(NMD_S_StR$data$StudyId))` publications and involving `r NMD_S_StR[["s.nlevels"]][1]` different animal strains.

|            Level            |                                               Number of categories for that level included in this analysis                                               |                                                                   Attributable variance                                                                   |
|:-------------:|:---------------------------:|:---------------------------:|
|           Strain            |                        `r ifelse('Strain' %in% NMD_S_StR$s.names, NMD_S_StR$s.nlevels[[which(NMD_S_StR$s.names == 'Strain')]], 0)`                        |                         `r ifelse('Strain' %in% NMD_S_StR$s.names, NMD_S_StR$sigma2[[which(NMD_S_StR$s.names == 'Strain')]], NA)`                         |
|       Study x Strain        |                `r ifelse('Strain/StudyId' %in% NMD_S_StR$s.names, NMD_S_StR$s.nlevels[[which(NMD_S_StR$s.names == 'Strain/StudyId')]], 0)`                |                 `r ifelse('Strain/StudyId' %in% NMD_S_StR$s.names, NMD_S_StR$sigma2[[which(NMD_S_StR$s.names == 'Strain/StudyId')]], NA)`                 |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_StR$s.names, NMD_S_StR$s.nlevels[[which(NMD_S_StR$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_StR$s.names, NMD_S_StR$sigma2[[which(NMD_S_StR$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
For other neurobiological outcomes, an NMD was calculable for `r df_S_OthN %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_OthN)` comparisons, i.e. `r ((df_S_OthN %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_OthN)))*100` % of comparisons.

The effect of exercise on other neurobiological outcomes in animals using NMD as the effect size is shown in Figure 2.8.5. The pooled estimate for NMD across all individual comparisons is displayed as a diamond shape at the bottom of the plot. Dotted lines indicate the prediction interval of the pooled estimate.

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
NMD_S_OthN <- run_ML_NMD(df, "TvC", "Other neurobiological", 0.5)
```

**Figure 2.8.5**

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height = 12}
forest_metafor_NMD(NMD_S_OthN, "Other neurobiological")
```

Exercise had a pooled effect on other neurobiological outcomes of NMD = `r NMD_S_OthN[["beta"]][1]` (95% CI: `r NMD_S_OthN[["ci.lb"]]` to `r NMD_S_OthN[["ci.ub"]]`) with a prediction interval of `r predict(NMD_S_OthN)$pi.lb` to `r predict(NMD_S_OthN)$pi.ub`). For reference the pooled effect size for SMD was `r SMD_S_OthN[["SMD_ML"]][["beta"]][1]` (95% CI: `r SMD_S_OthN[["SMD_ML"]][["ci.lb"]]` to `r SMD_S_OthN[["SMD_ML"]][["ci.ub"]]`).

`r NMD_S_OthN[["k"]]` experimental comparisons were reported in `r NMD_S_OthN[["s.nlevels"]][3]` experiments reported from `r length(unique(NMD_S_OthN$data$StudyId))` publications and involving `r NMD_S_OthN[["s.nlevels"]][1]` different animal strains.

|            Level            |                                                Number of categories for that level included in this analysis                                                 |                                                                    Attributable variance                                                                     |
|:-------------:|:---------------------------:|:---------------------------:|
|           Strain            |                        `r ifelse('Strain' %in% NMD_S_OthN$s.names, NMD_S_OthN$s.nlevels[[which(NMD_S_OthN$s.names == 'Strain')]], 0)`                        |                         `r ifelse('Strain' %in% NMD_S_OthN$s.names, NMD_S_OthN$sigma2[[which(NMD_S_OthN$s.names == 'Strain')]], NA)`                         |
|       Study x Strain        |                `r ifelse('Strain/StudyId' %in% NMD_S_OthN$s.names, NMD_S_OthN$s.nlevels[[which(NMD_S_OthN$s.names == 'Strain/StudyId')]], 0)`                |                 `r ifelse('Strain/StudyId' %in% NMD_S_OthN$s.names, NMD_S_OthN$sigma2[[which(NMD_S_OthN$s.names == 'Strain/StudyId')]], NA)`                 |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_OthN$s.names, NMD_S_OthN$s.nlevels[[which(NMD_S_OthN$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% NMD_S_OthN$s.names, NMD_S_OthN$sigma2[[which(NMD_S_OthN$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
